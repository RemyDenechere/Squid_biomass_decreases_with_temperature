function param = baseparameters(WinfSquid, A_squid)
...
% PLot parameters: 
param.SpId = {'SmallPel','Mesopelagic','LargePel', 'Demersal', 'Squid'};  % Name of the functional groups
param.LWidth = [1.5, 1.5, 3, 3, 3]; % plot: Line width for each the functional groups 
param.Color = [0.07, 0.62, 1 ; ...  % Plot: Color for the functional groups 
    0.85, 0.33, 0.10; ...
    0.00, 0.45, 0.74; ...
    0, 0, 0 ;...
    1.00,0.55,0.00]; 

param.tEnd =  150;     % Number of time step for ODE solver
...
% Index for functional groups:  
param.nSpecies = length(param.ix1);         % 
...
param.A_squid = A_squid;
param.h = zeros(1, param.NbSizeGrp); param.h(1:end) = 20; % Value fish
param.h(param.ix1(5):param.ix2(5)) = param.A_squid/(param.epsAssim*(0.6 - 0.2)); % Value ceph
